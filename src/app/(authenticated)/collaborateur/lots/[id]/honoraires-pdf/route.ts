...
export async function GET(request: Request, ctx: RouteContext) {
  ...
  // Contrôle d'accès identique aux autres écrans lot.
  const lot = await findLotForUser(id, me.role);
  if (!lot) notFound();
  ...
  // Chargement des données du dossier actif du lot, nécessaires au document.
  if (!lot.dossierId) {
    return erreur("Ce lot n'a pas de client (acquéreur).", 400);
  }
  ...
  const data: HonorairesPdfData = {
    ...
    lots: [dossier.lot],
    programmeName: dossier.lot.programme.name,
    programmeAddress: formatAdresseProgramme(dossier.lot.programme),
    notaireNom: notaire
