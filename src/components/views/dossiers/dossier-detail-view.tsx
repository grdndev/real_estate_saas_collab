...
/**
 * Vue « fiche dossier » — implémentation unique partagée par l'espace
 * collaborateur et l'espace admin (T5/T15).
 *
 * Le contrôle d'accès au dossier est fait par la route appelante
 * (`findDossierForUser`) ; la vue ne connaît pas le rôle de l'utilisateur.
 */
interface Props {
  dossierId: string;
...
export async function DossierDetailView({
  dossierId: id,
  currentUserId,
  basePath,
}: Props) {
  const [dossier, pendingClients, notaries, unreadMessages] = await Promise.all(
    [
      ...
      prisma.user.findMany({
        where: {
          role: "CLIENT",
          // Tout client déjà utilisable et non rattaché à un dossier actif : les clients
          // inscrits + confirmés (PENDING_ASSOCIATION) mais aussi ceux créés/importés
          // par l'équipe (ACTIVE). Les dossiers archivés (historique) ne bloquent pas.
  ...
  // Un dossier archivé est un historique : lecture seule (T10).
  const isArchived = dossier.archivedAt != null;
  // Client associé sans accès à la plateforme (T7).
  const clientHasNoAccount = dossier.client?.status === "NO_ACCOUNT";
  const clientEmail = displayableEmail(dossier.client?.email);
          ...
          <div>
            <p className="text-equatis-night-700 flex items-center gap-2 text-xs uppercase">
              {dossier.client
                ...
                : "Dossier sans client"}
              {clientHasNoAccount && (
                <Badge variant="neutral">Sans compte</Badge>
              )}
            </p>
            ...
            <p className="mt-2 flex flex-wrap items-center gap-2">
              ...
              {isArchived && <Badge variant="neutral">Dossier archivé</Badge>}
            </p>
            {isArchived && (
              <p className="mt-2 max-w-prose text-xs text-slate-500">
                Ce dossier est l&apos;historique d&apos;un client dissocié :
                messages, documents et timeline sont conservés en lecture. Il
                sera restitué si ce client est réassocié au même lot.
              </p>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <Card>
            ...
            <CardContent>
              <Link href={`${basePath}/${dossier.id}/messagerie`}>
              ...
              {clientHasNoAccount && (
                <p className="mt-2 text-xs text-slate-500">
                  Ce client n&apos;a pas d&apos;accès à la plateforme : il ne
                  peut ni lire ni écrire de message.
                </p>
              )}
            </CardContent>
          </Card>
              ...
              {dossier.clientId ? (
                <>
                  <div className="space-y-2 text-sm">
                    ...
                    {clientHasNoAccount && (
                      <p className="text-xs text-slate-500">
                        Client associé : pas d&apos;accès à la plateforme, ni
                        invitation, ni email de relance, ni messagerie.
                      </p>
                    )}
                  </div>
                  {dossier.client && !clientHasNoAccount && (
                  ...
                  {clientHasNoAccount && dossier.clientId && (
                    <div className="border-t border-slate-100 pt-4">
                  ...
                  <div className="border-t border-slate-100 pt-4">
                    <Link
                      href={`${basePath}/${dossier.id}/fiche-client`}
