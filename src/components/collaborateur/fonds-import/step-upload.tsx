...
import { parseFondsFileAction } from "@/lib/collaborateur/fonds-import-actions";
import type {
  ParsedFondsLot,
...
export function StepUpload({ onParsed }: Props) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      ...
      const result = await parseFondsFileAction(b64);
      if (!result.ok) {
        setError(result.error);
        return;
      }
      const { rows, appelTypes, errors } = result.value;
      if (rows.length === 0) {
        setError(
          errors.length > 0
            ? errors.join(" ")
      ...
      setWarnings(errors);
      onParsed(b64, rows, appelTypes, errors);
    });
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-slate-600">
        Déposez le tableau de suivi des appels de fonds Excel.
      </p>
